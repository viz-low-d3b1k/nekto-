...
function injectMenu() {
    ...
    document.getElementById('saveInterval').addEventListener('click', () => {
        ...
        if (!isNaN(newInterval) && newInterval >= 1000 && newInterval <= 5000) {
            ...
            clearInterval(checkConversationEndInterval);
            checkConversationEndInterval = setInterval(checkConversationEnd, checkInterval);
...
function handleKeyboardShortcuts(event) {
    ...
    if (event.key === 'ArrowUp') {
        ...
        console.log("Нажата клавиша Влево. Подтверждение действия.");
...
window.onload = () => {
    ...
    applyCustomStyles();
    setInterval(checkConversationAndSwalPresence, 1000);
